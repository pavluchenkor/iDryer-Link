...
In short, the question was this: LINK knows the Wi-Fi password, and once it is connected to a local MQTT broker it gains access to the home automation setup, and all of that comes down to trusting the project author.
...
At the same time, the project is not built on blind trust in me. This is a private project, not a large team. That is exactly why the LINK source code is open: structure, comments and documentation have been brought to a state other developers can work with.
...
The bottom line is simple: the project is open, its architectural decisions are deliberate, its mainstream scenario is cloud-based, and for those who want a different level of control there is source code and the ability to adapt the firmware to their own requirements.
...
Если коротко, вопрос был такой: Link знает пароль от Wi-Fi, а при подключении к локальному MQTT-брокеру получает доступ к домашней автоматизации, и всё это упирается в доверие к автору проекта.
...
При этом проект не строится на слепом доверии ко мне. Это частный проект, а не большая команда. Именно поэтому исходный код LINK открыт: структура, комментарии и документация приведены в состояние, пригодное для работы другим разработчикам.
...
Итог простой: проект открыт, архитектурные решения в нём осознанные, массовый сценарий у продукта облачный, а для тех, кому нужен другой уровень контроля, есть исходный код и возможность адаптировать прошивку под свои требования.
